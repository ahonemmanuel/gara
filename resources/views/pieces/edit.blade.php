...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('pieces.update', $piece) }}" method="POST" enctype="multipart/form-data" id="pieceForm">
            ...
            {{-- Nouveau champ pour nouvelle marque (caché par défaut) --}}
            ...
            {{-- Nouveau champ pour nouveau modèle (caché par défaut) --}}
            ...
            <div class="mb-3">
                ...
                <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" rows="4">{{ old('description', $piece->description) }}</textarea>
                @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="prix" id="prix" class="form-control @error('prix') is-invalid @enderror" value="{{ old('prix', $piece->prix) }}" min="500" step="0.01">
                @error('prix')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="compatible_avec" id="compatible_avec" class="form-control @error('compatible_avec') is-invalid @enderror" rows="2" placeholder="Ex: Toyota Corolla 2015-2020, Honda Civic 2016-2021">{{ old('compatible_avec', $piece->compatible_avec) }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <div class="row g-2">
                        @foreach($piece->photos as $photo)
                            <div class="col-md-2">
                                <img src="{{ asset('storage/' . $photo) }}" class="img-thumbnail" alt="Photo de la pièce">
                            </div>
                        @endforeach
                    </div>
                    <small class="text-muted">Télécharger de nouvelles photos remplacera les anciennes</small>
                </div>
            @endif

            {{-- Nouvelles Photos --}}
            <div class="mb-3">
                <label for="photos" class="form-label">Nouvelles photos (plusieurs possibles)</label>
            ...
            <div class="form-check mb-4">
                ...
                <label for="disponible" class="form-check-label">Disponible à la vente</label>
            </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                marqueSelect.addEventListener('change', function() {
                    ...
                    const currentModeleId = {{ old('modele_id', $piece->modele_id) ?? 'null' }};
                    ...
                    fetch(`/api/marques/${marqueId}/modeles`)
                        ...
                        .then(data => {
                            ...
                            data.forEach(modele => {
                                ...
                                // Pré-sélectionner le modèle actuel
                ...
                // Charger les modèles au chargement de la page si une marque est sélectionnée
