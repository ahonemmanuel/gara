            ...
            <table class="min-w-full leading-normal">
                ...
                @foreach($commandes as $commande)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p class="text-gray-900 whitespace-no-wrap font-bold">{{ number_format($commande->total, 2) }} €</p>
                        </td>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                    ...
                                    @foreach($commande->pieces as $piece)
                                        <div class="flex justify-between items-center border-b pb-2">
                                            ...
                                            <div class="text-right">
                                                <p>{{ $piece->pivot->quantite }} x {{ number_format($piece->pivot->prix_unitaire, 2) }} €</p>
                                                <p class="font-bold">{{ number_format($piece->pivot->quantite * $piece->pivot->prix_unitaire, 2) }} €</p>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                <div class="mt-3 pt-3 border-t">
                                    <p class="text-right font-bold text-lg">Total: {{ number_format($commande->total, 2) }} €</p>
                                </div>
                            </div>
                        </td>
                    </tr>
